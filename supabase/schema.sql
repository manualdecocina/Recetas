...
create unique index recipes_public_path_key on public.recipes (public_path);
...
create function public.sync_public_path()
returns trigger
language plpgsql
...
begin
  if new.public_path is null or new.public_path = '' then
    if new.source_url is not null and (new.source_url like 'https://manualdecocina.com/%' or new.source_url like 'https://www.manualdecocina.com/%') then
...
begin
  ...
  insert into public.recipes (
    ...
    gen_random_uuid(), p_language, p_slug, null, p_title, p_excerpt, p_ingredients, p_steps,
...
begin
  ...
  insert into public.recipes (
    ...
    v_group, p_language, p_slug, null, p_title, p_excerpt, p_ingredients, p_steps,
    p_category, p_prep_time_minutes, p_cook_time_minutes, p_servings, p_image_url, p_published
  ) returning * into v_row;
